...
class DeepSeekMegaBlocksAdapter(nn.Module):
    def __init__(self, config: DeepseekV3Config, module: DeepseekV3MoE, ep_group: dist.ProcessGroup):
        super().__init__()

        ep_rank = dist.get_rank(ep_group)
        ep_size = dist.get_world_size(ep_group)

        assert config.n_routed_experts % ep_size == 0, "Number of experts must be divisible by the number of expert parallel groups"
        num_experts_per_rank = config.n_routed_experts // ep_size
