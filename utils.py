...
def assert_gradients_are_the_same(model, world_size, rank):
    except_patterns = model.except_patterns if hasattr(model, "except_patterns") else []
...
def apply_expert_parallelism(model, ep_group, impl="mega_blocks"):
    ...
    if isinstance(model, MixtralForCausalLM):
        ...
        if impl == "mega_blocks":
            ...
            from mixtral import MixtralSparseMoeBlockEP
